...
- One-Click Auto Research UX: на главной и `/matches` есть кнопка `Обновить всё доступное автоматически`; на странице матча есть `Подготовить прогноз` и `Создать data pack`.
...
- Sync в MVP 0.4.2 запускается только вручную через кнопки, `/admin/imports` или CLI scripts. Page-load sync запрещён.
...
Основной путь пользователя в MVP 0.4.2:

1. Откройте главную страницу `/` и нажмите `Обновить всё доступное автоматически`.
2. Сайт server-side выполнит PandaScore Free Fixtures, Valve Rankings, Steam/CS Updates, snapshots, predictions и обновление research queue.
3. Выберите матч в `/matches` или `/predictions`.
4. На странице матча нажмите `Подготовить прогноз`.
5. Если статус остаётся `Не готов` или `Слабый сигнал`, нажмите `Создать data pack` и добавьте ручные реальные данные: состав, player stats, map stats, veto, H2H и новости.
...
MVP 0.4.2 сохраняет отдельный слой видимости и приоритизации матчей. Он не удаляет данные и не влияет на математическую уверенность прогноза: это только UI/filtering слой поверх сохранённых матчей.
...
MVP 0.4.2 сохраняет controlled analyst workflow. Sample pack остаётся dev-only proof-flow, а manual real pack — отдельный путь для вручную проверенных реальных данных по одному выбранному матчу: research queue -> validate -> preview -> apply -> snapshots -> prediction audit -> readiness before/after.
...
MVP 0.4.2 не утверждает, что реальный прогноз получен, пока пользователь не внёс валидный `manual_real`, `parsed_demo` или GRID-style data pack. Wizard может быть полностью готов технически, но статус `First real manual_real forecast ready` остаётся `no`, если реальные данные не применялись.
...
- readiness `>= L3`;
- источник не sample-only и содержит `manual_real`, `parsed_demo` или GRID-style data;
- нет critical `needs_review`/source conflict;
- есть player/map/veto coverage;
- `dataQualityScore >= 50`;
- `Manual Real Pack Quality >= 65`.
...
MVP 0.4.2 добавляет отдельный слой новостей и событий, который влияет на `risk`, `confidence` и ограниченно на probability. Новости не перебивают математические факторы и не используются как betting/odds signal.
...
MVP 0.4.2 сохраняет научный слой поверх rule-based prediction:
...
- В MVP 0.4.2 поддерживается `parsed_demo` JSON import для `PlayerStatSnapshot`, `TeamMapStat`, `TeamFormSnapshot` и round/economy proxies.
- Actual `.dem` parser worker переносится в MVP 0.4.3+. Рекомендуемый кандидат: AWPy (`pip install awpy`, Python >= 3.11), который умеет читать CS2 demo через `Demo(...).parse()` и отдавать rounds/kills/damages/shots/bomb/grenades/ticks. Загрузка demo должна быть ручной/локальной, без HLTV scraping.
...
Автоматические источники вынесены в adapter layer. MVP 0.4.2 использует free-first priority:
...
MVP 0.4.2 не запускает синхронизацию при открытии страниц. Это сделано специально: page-load sync может тормозить dashboard и быстро упереться в rate limits. Обновление запускается только кнопками или CLI.
