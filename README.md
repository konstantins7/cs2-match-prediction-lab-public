...
## Public Demo / FACEIT API Review

CS2 Match Prediction Lab is a non-betting research analytics MVP for explainable Counter-Strike 2 match analysis.

The project does not provide odds, betting advice, staking recommendations, or guaranteed outcomes. It is focused on data coverage, prediction readiness, transparent factor analysis, source quality, and explainable probabilistic reporting.

FACEIT integration is optional and intended only for public player/team/competition/statistics context. FACEIT API calls are server-side only. API keys must be stored only in local `.env` files and are never committed to the repository. No API keys are included in this public demo.

The project can run without FACEIT, GRID, Liquipedia, or PandaScore keys in basic/local demo mode. In that mode, it uses demo/seed data, manual data packs, parsed demo JSON intake, readiness gates, forecast autopilot, research queue, and model-lab views.
...
- API keys are not stored in source code, README, tests, logs, or build artifacts.
...
- Sync в MVP 0.4.6 запускается только вручную через кнопки, `/admin/imports` или CLI scripts. Page-load sync запрещён.
...
Основной путь пользователя в MVP 0.4.6:
...
MVP 0.4.6 сохраняет отдельный слой видимости и приоритизации матчей. Он не удаляет данные и не влияет на математическую уверенность прогноза: это только UI/filtering слой поверх сохранённых матчей.
...
MVP 0.4.6 не утверждает, что реальный прогноз получен, пока пользователь не внёс валидный `manual_real`, `parsed_demo` или confirmed GRID data pack. Workflow может быть полностью готов технически, но статус `First real forecast ready` остаётся `no`, если реальные данные не применялись.
...
MVP 0.4.6 сохраняет отдельный слой новостей и событий, который влияет на `risk`, `confidence` и ограниченно на probability. Новости не перебивают математические факторы и не используются как betting/odds signal.
...
MVP 0.4.6 сохраняет научный слой поверх rule-based prediction:
...
- В MVP 0.4.6 поддерживается `parsed_demo` JSON import для `PlayerStatSnapshot`, `TeamMapStat`, `TeamFormSnapshot` и round/economy proxies.
...
Автоматические источники вынесены в adapter layer. MVP 0.4.6 использует free-first priority:
...
MVP 0.4.6 не запускает синхронизацию при открытии страниц. Это сделано специально: page-load sync может тормозить dashboard и быстро упереться в rate limits. Обновление запускается только кнопками или CLI.
