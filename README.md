...
- First Real Forecast Pack Workflow: MVP 0.7.1 добавляет строгий путь `manual_real_pack` для первого настоящего прогноза без fake data и без `analyst_sample` как real.
...
- Sync в MVP 0.7.1 запускается только вручную через кнопки, `/admin/imports` или CLI scripts. Page-load sync запрещён.
...
MVP 0.7.1 делает главный путь похожим на современный esport analytics dashboard, а не на dev/admin tool.
...
MVP 0.7.1 добавляет workflow `Собрать первый реальный прогноз` на странице матча и в `/admin/research-queue`.
...
MVP 0.7.1 добавляет слой `Где взять недостающие данные`: пользователь видит не просто “данных нет”, а легальные пути получить roster, player stats, map/veto, H2H, news, round/economy и ranking.
...
Import profiles в 0.7.1 являются JSON-first:
...
MVP 0.7.1 добавляет dedicated JSON-first intake для prepared demo/stat exports. Это не raw `.dem` parser worker: приложение принимает только JSON, который пользователь подготовил во внешнем tool или вручную нормализовал в canonical format.
...
MVP 0.7.1 добавляет CSV-first import для `manual_real` данных. Это удобный табличный вход поверх существующего `manual_real_pack` flow: пользователь скачивает шаблон, заполняет его в Excel / Google Sheets / WPS, сохраняет как CSV/TSV или вставляет copy-paste table, затем запускает Validate -> Preview -> Apply.
...
XLSX parser, SQL import и raw `.dem` worker остаются future/inactive. В MVP 0.7.1 нет новых heavy parser dependencies, нет scraping, нет page-load sync, и forecast math / Real Forecast Ready gates не меняются.

## Как пользоваться сайтом

Основной путь пользователя в MVP 0.7.1:
...
MVP 0.7.1 сохраняет отдельный слой видимости и приоритизации матчей. Он не удаляет данные и не влияет на математическую уверенность прогноза: это только UI/filtering слой поверх сохранённых матчей.
...
MVP 0.7.1 сохраняет controlled analyst workflow. Sample pack остаётся dev-only proof-flow, а manual real pack — отдельный путь для вручную проверенных реальных данных по одному выбранному матчу: research queue -> validate -> preview -> apply -> snapshots -> prediction audit -> readiness before/after.
...
MVP 0.7.1 не утверждает, что реальный прогноз получен, пока пользователь не внёс валидный `manual_real`, `parsed_demo` или confirmed GRID data pack. Workflow может быть полностью готов технически, но статус `First real forecast ready` остаётся `no`, если реальные данные не применялись.
...
MVP 0.7.1 сохраняет отдельный слой новостей и событий, который влияет на `risk`, `confidence` и ограниченно на probability. Новости не перебивают математические факторы и не используются как betting/odds signal.
...
MVP 0.7.1 сохраняет научный слой поверх rule-based prediction:
...
- В MVP 0.7.1 поддерживается `parsed_demo` JSON import для `PlayerStatSnapshot`, `TeamMapStat`, `TeamFormSnapshot` и round/economy proxies.
...
Автоматические источники вынесены в adapter layer. MVP 0.7.1 использует free-first priority:
...
MVP 0.7.1 не запускает синхронизацию при открытии страниц. Это сделано специально: page-load sync может тормозить dashboard и быстро упереться в rate limits. Обновление запускается только кнопками или CLI.
