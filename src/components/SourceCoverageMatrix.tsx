...
export function SourceCoverageMatrix({ rows, compact = false }: { rows: SourceCoverageRow[]; compact?: boolean }) {
  ...
  return (
    <section className="rounded border border-lab-border bg-lab-panel p-4">
      <div>
        <h2 className="font-semibold text-white">Source Coverage Matrix</h2>
              ...
              <tr key={row.dataType}>
                ...
                {row.cells.map((cell) => (
                  <td key={`${row.dataType}-${cell.source}`} className="px-3 py-3 align-top">
                    <p className={statusClass[cell.status] ?? "text-lab-muted"}>{cell.status}</p>
                    <p className="mt-1">Used: {cell.usedInPrediction ? "yes" : "no"}</p>
