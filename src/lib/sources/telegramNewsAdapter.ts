...
export const telegramNewsAdapter: SourceAdapter = {
  ...
  status() {
    ...
    return buildSourceStatus({
      ...
      configured: enabled,
      message: enabled
        ? "Telegram news sync skeleton is enabled, but MVP 0.7.2 supports only official API/bot/user-approved sources. No scraping/private channels."
        : "Disabled by default. Telegram insider signals are manual/reference-only in MVP 0.7.2.",
  ...
  async sync(context) {
    ...
    return resultFromRecords({
      ...
      records: [],
      status: "blocked",
      errors: ["Telegram automatic collection is not implemented in MVP 0.7.2. Use official API/bot/user-approved manual imports only."],
